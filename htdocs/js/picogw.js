...
!function(a, b) {
    ...
    'use strict'; function a(a, b) {
        let c, d=document.createElement(a||'div'); for (c in b)d[c]=b[c]; return d;
    } function b(a) {
        for (let b=1, c=arguments.length; c>b; b++)a.appendChild(arguments[b]); return a;
    } function c(a, b, c, d) {
        let e=['opacity', b, ~~(100*a), c, d].join('-'), f=.01+c/d*100, g=Math.max(1-(1-a)/b*(100-f), a), h=j.substring(0, j.indexOf('Animation')).toLowerCase(), i=h&&'-'+h+'-'||''; return m[e]||(k.insertRule('@'+i+'keyframes '+e+'{0%{opacity:'+g+'}'+f+'%{opacity:'+a+'}'+(f+.01)+'%{opacity:1}'+(f+b)%100+'%{opacity:'+a+'}100%{opacity:'+g+'}}', k.cssRules.length), m[e]=1), e;
    } function d(a, b) {
        let c, d, e=a.style; if (b=b.charAt(0).toUpperCase()+b.slice(1), void 0!==e[b]) return b; for (d=0; d<l.length; d++) if (c=l[d]+b, void 0!==e[c]) return c;
    } function e(a, b) {
        for (let c in b)a.style[d(a, c)||c]=b[c]; return a;
    } function f(a) {
        for (let b=1; b<arguments.length; b++) {
            let c=arguments[b]; for (let d in c) void 0===a[d]&&(a[d]=c[d]);
        } return a;
    } function g(a, b) {
        ...
        function c(b, c) {
            ...
            } var i, j=d.scale*(d.length+d.width), k=2*d.scale*j, l=-(d.width+d.length)*d.scale*2+'px', m=e(f(), {position: 'absolute', top: l, left: l}); if (d.shadow) for (i=1; i<=d.lines; i++)h(i, -2, 'progid:DXImageTransform.Microsoft.Blur(pixelradius=2,makeshadow=1,shadowopacity=.3)'); for (i=1; i<=d.lines; i++)h(i); return b(a, m);
        }, h.prototype.opacity=function(a, b, c, d) {
            let e=a.firstChild; d=d.shadow&&d.lines||0, e&&b+d<e.childNodes.length&&(e=e.childNodes[b+d], e=e&&e.firstChild, e=e&&e.firstChild, e&&(e.opacity=c));
        };
    } var j, k, l=['webkit', 'Moz', 'ms', 'O'], m={}, n={lines: 12, length: 7, width: 5, radius: 10, scale: 1, corners: 1, color: '#000', opacity: .25, rotate: 0, direction: 1, speed: 1, trail: 100, fps: 20, zIndex: 2e9, className: 'spinner', top: '50%', left: '50%', shadow: !1, hwaccel: !1, position: 'absolute'}; if (h.defaults={}, f(h.prototype, {spin: function(b) {
        this.stop(); let c=this, d=c.opts, f=c.el=a(null, {className: d.className}); if (e(f, {position: d.position, width: 0, zIndex: d.zIndex, left: d.left, top: d.top}), b&&b.insertBefore(f, b.firstChild||null), f.setAttribute('role', 'progressbar'), c.lines(f, c.opts), !j) {
            let g, h=0, i=(d.lines-1)*(1-d.direction)/2, k=d.fps, l=k/d.speed, m=(1-d.opacity)/(l*d.trail/100), n=l/d.lines; !function o() {
                h++; for (let a=0; a<d.lines; a++)g=Math.max(1-(h+(d.lines-a)*n)%l*m, d.opacity), c.opacity(f, a*d.direction+i, g, d); c.timeout=c.el&&setTimeout(o, ~~(1e3/k));
            }();
        } return c;
    }, stop: function() {
        let a=this.el; return a&&(clearTimeout(this.timeout), a.parentNode&&a.parentNode.removeChild(a), this.el=void 0), this;
    }, lines: function(d, f) {
        function h(b, c) {
        ...
        k=function() {
            let c=a('style', {type: 'text/css'}); return b(document.getElementsByTagName('head')[0], c), c.sheet||c.styleSheet;
        }(); let o=e(a('group'), {behavior: 'url(#default#VML)'}); !d(o, 'transform')&&o.adj?i():j=d(o, 'animation');
    } return h;
});
