        ...
        onconnect: (_picogw)=>{
            ...
            picogw.callproc({
                ...
                for (let dev in devhash) {
                    if (dev.indexOf(DevNamePrefix.toLowerCase()) == 0) {
                        devlist.push(dev);
                    }
                }
                ...
                Promise.all(devlist.map((dev)=>new Promise((ac, rj)=>{
                    ...
                    for (let path in devices) {
                        ...
                        DescriptionProperties.forEach((elem)=>{
                            ...
                            try {
                                desc += ` ${elem} = ${devices[path][elem].cacheValue} :`;
                            } catch (e) {}
                        });
...
function simple_enum_setup(dev_path, propname, cache) {
    ...
    picogw.sub(dev_path+'/'+propname.toLowerCase(), (re)=>{
        let cval = re[dev_path+'/'+propname.toLowerCase()].value;
...
function slider_setup(dev_path, propName, cache) {
    if (cache != null) {
        $(`#${propName}`).attr('value', cache.cacheValue);
    ...
    picogw.sub(dev_path+'/'+propName.toLowerCase(), (re)=>{
        $(`#${propName}`).val(
            re[dev_path+'/'+propName.toLowerCase()].value
        ).slider('refresh');
    });
...
$('#controlpage').on('pagebeforeshow', function(event) {
    ...
    function setupControls() {
        ...
        SimpleEnumProperties.forEach((pname)=>{
            set_handlers[pname.toLowerCase()]
                = simple_enum_setup(dev_path, pname, devCache[pname.toLowerCase()]);
        });

        // Slider properties
        SliderProperties.forEach((pname)=>{
            set_handlers[pname.toLowerCase()]
                = slider_setup(dev_path, pname, devCache[pname.toLowerCase()]);
        });
    }
...
$(document).on('change', '[type="radio"]', function() {
    try {
        set_handlers[this.name.toLowerCase()](this.value);
    } catch (e) {}
});
...
SliderProperties.forEach((pname)=>{
    $('#frm').on('slidestop', '#'+pname, function() {
        try {
            set_handlers[this.name.toLowerCase()](parseInt(this.value));
        } catch (e) {}
    });
});
